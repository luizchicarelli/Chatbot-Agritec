...
try:
    genai.configure(api_key="AIzaSyD08IuFG3--9AIpimoICHClRCgXQYthngg") # 👈 Coloque sua chave válida
...
def responder_com_gemini(pergunta, intencao, dados_api, primeira_pergunta):
    ...
    if GEMINI_MODEL is None:
        return "⚠️ O assistente de IA está indisponível. Verifique a configuração do token do Gemini."
...
# --- LÓGICA PRINCIPAL DO CHATBOT (CORRIGIDA) ---
def chatbot():
    print("🌾 Bem-vindo ao Chatbot Agritec (v5.2 - Estável e Integrado com Gemini)!")
    ...
    print("   - O que é ferrugem asiática da soja?")
    print("   - Qual o melhor fungicida Priori para soja?")
    print("   - Qual a praga Spodoptera?")
    print("   - O que é um Agrometeorologista?") # Exemplo de pergunta para autonomia do Gemini

    if not ACCESS_TOKEN:
        print("\n🛑 Falha na autenticação Embrapa. Verifique seu token."); return
    if not carregar_dados_da_api():
        print("\n🛑 Falha ao carregar dados iniciais."); return
    ...
    while True:
        ...
        if data_plantio:
            try:
                ...
                if data_plantio_obj > date.today():
                    print("❓ A data de plantio não pode ser no futuro."); continue
            except ValueError:
                print("❓ Formato de data inválido. Use DD/MM/AAAA."); continue
        ...
        if intencao in ["zoneamento", "produtividade", "cultivares"]:
            ...
            # Validações de cultura (Sempre necessárias para Agritec)
            if not cultura:
                # SE FALTA CULTURA, CAI NO FALLBACK
                intencao = "responde_agro"
            elif not id_cultura_encontrada:
                # Se a cultura existe na pergunta, mas não na base da API, emitimos erro
                dados_api_brutos = f"❌ A cultura '{cultura}' não foi encontrada em nossa base de dados Agritec."

            # Validações Geográficas (Se cultura OK, mas falta localização, muda a intenção para fallback)
            if not dados_api_brutos:
                if intencao in ["zoneamento", "produtividade"]:
                    if not cidade:
                        # 💥 CORREÇÃO: Força o fallback para o Gemini responder sobre a cultura de forma geral
                        intencao = "responde_agro"
                    else:
                        codigo_ibge = CIDADES_COM_DADOS.get(cidade)
                        if not codigo_ibge:
                            dados_api_brutos = f"❌ Não encontrei o código IBGE para a cidade '{cidade.title()}'."
                    ...
                    if not uf:
                        # 💥 CORREÇÃO: Força o fallback para o Gemini responder sobre a cultura
                        intencao = "responde_agro"
                    else:
        ...
        # ==========================================================
            ...
            if not cidade:
                intencao = "responde_agro" # Força fallback
            else:
                dados_api_brutos = consultar_clima_por_cidade(cidade)

        # ==========================================================
        # 3. AÇÕES SMARTSOLOS (Demonstração de Classificação)
        # ==========================================================
        elif intencao == "solos":
        ...
        # ==========================================================
            ...
            if not nome_produto:
                dados_api_brutos = "❌ Não foi possível identificar o nome do produto para consulta. Tente ser mais específico."
            else:
            ...
            if not nome_praga:
                dados_api_brutos = "❌ Não foi possível identificar o nome da praga para consulta. Tente ser mais específico."
            else:
                dados_api_brutos = consultar_praga(nome_praga)

        # ==========================================================
        # 5. FALLBACK (RESPONDEAGRO) - Padrão e acionado por validação
        # ==========================================================
        ...
        if dados_api_brutos is not None:
            resposta_final = responder_com_gemini(
                ...
                intencao,
                dados_api_brutos,
                contador_perguntas == 1 # Apenas a primeira pergunta recebe a saudação
            )
            print(f"\n🤖 AgroAssist:\n{resposta_final}")
        else:
            print("\n❌ Não foi possível gerar uma resposta. Erro interno: dados_api_brutos é None.")
